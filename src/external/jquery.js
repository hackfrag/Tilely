...
jQuery.extend({
	...
	require: function(scripts, callback, cache, findOrMake, length) {

		scripts  	= scripts.push ? scripts : [scripts.substring ? {src: scripts} : scripts];
		callback	= typeof callback === "function" ? callback : jQuery.noop;
		cache 		= jQuery.require.cache || {};
		findOrMake	= function(key) { return cache[key] || { loaded: false, queue: {} } };
		...
		jQuery.each(scripts, function(satisfied, script) {
			...
			script.requires = script.requires ?
					(typeof script.requires === "string" ?
							[script.requires] : script.requires) : [];

			jQuery.each(script.requires, function(j, requirement){

				cache[requirement] = findOrMake(requirement);

				if ( !(satisfied = cache[requirement].loaded) ) {
					cache[requirement].queue[script.src] = script;
				}
